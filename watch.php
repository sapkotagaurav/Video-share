...
if(isset($_GET['id']))
{
	...
	$query2 = $con->query("SELECT COUNT('id') FROM `videosname`");
...
<a href="logout.php" class="btn btn-danger" style="left:50%"> Log out</a>
	...
	<video width = "800"; height = auto; align = center; autoplay controls src="http://localhost/term_project_css/uploaded/<?php echo $row['name']?>" id="video" name="video" class="video"></video>
	<br>
	</section> 
	<br><br>
...
video.onended = (event) => {
	window.location.href = 'http://localhost/term_project_css/watch.php?id=<?php while( in_array( ($n = mt_rand(1,$row2[0])), array($id) ) );echo $n;?>';
};</script>
</body>
</html>
